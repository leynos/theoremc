...
`Actions` declarations in `TheoremDoc`. The macro must not infer signatures
from `crate::theorem_actions`, because doing so would accept drift rather than
detect it. The implementation uses `theoremc_core::collision::referenced_actions`
to walk action references in the same order as collision detection, then uses
`theoremc_core::mangle::mangle_action_name` to resolve each canonical action
to its Rust export.
