...
| Tests                | `make test`                                                                                        | `cargo nextest run --workspace --all-targets --all-features`       |
...
`make test` is intentionally nextest-backed. Override `NEXTEST_FLAGS` only with
options supported by cargo-nextest; add doctest-specific checks as separate
targets rather than passing `cargo test`-only flags through `make test`.
