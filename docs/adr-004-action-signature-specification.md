...
- `let _: fn(graph: &mut Graph, node: NodeId) -> Result<(), AttachError> = action;`
  compiles when the function matches and fails with `E0308` when a parameter
  or return type drifts.
